...
//  UnsupportedHandler onUnsupported;
  ...
  /// CUSTOM CODE
  Future<ArCorePose> getCameraPose() async {
    final result = await _channel.invokeMethod('getCameraPose');
    return ArCorePose.fromMap(result);
  }

  /// CUSTOM CODE
  Future<Matrix4> getViewMatrix() async {
    final result = await _channel.invokeMethod('getViewMatrix');
    return Matrix4.fromList(result["viewMatrix"]);
  }

  /// CUSTOM CODE
  Future<Matrix4> getProjectionMatrix() async {
    final result = await _channel.invokeMethod('getProjectionMatrix');
    return Matrix4.fromList(result["projectionMatrix"]);
  }
